...
// --- THE ULTIMATE CLOCK SYNC FIX ---
// Added a 60-second buffer to synchronize Render's clock with Firebase's clock.
// This prevents server-wake-up overlap spam.
const SERVER_START_TIME = Date.now() - 60000; 
...
// ============================================================================
// THE DUPLICATE KILLER: NEUTRALIZED API ENDPOINT
// ============================================================================
app.post('/send-push', (req, res) => {
    // Because your frontend sometimes still sends fetch requests, 
    // AND your backend listens to the database, you were getting double notifications.
    // We return 200 OK so the frontend doesn't crash, but we DO NOT send a push here.
    // The Firestore Database listener below will securely handle 100% of the notifications.
    res.status(200).json({ success: true, message: "Intercepted to prevent duplicate. Delegated to DB listener." });
});
            ...
            if (change.type === 'added') {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        // ULTIMATE DUPLICATE TARGET FIX: Ensure the array has completely unique, valid IDs
